...
int main(int argc, char* argv[])
{
	...
	if (mode == 1)
	{
		for (int i = 0; i <= opt1 * 1 - 1; i++)
		{
			...
			string str2 = "./opt -S -o bench.ll benchmark.ll ";
			str2 = str2 + optimisations[i % opt1];
			const char* c2 = str2.c_str();
			string str3 = "clang bench.ll -o bench -lstdc++ -lm";
			const char* c3 = str3.c_str();
			system(c);
			system(c2);
			system(c3);
			string str4 = "./bench ";
			str4 = str4 + argv[2] + " " + optimisations[i % opt1];
	...
	else if (mode == 2)
	{
		for (int i = 0; i <= opt2 * 1 - 1; i++)
		{
			...
			const char* c2 = str2.c_str();
			string str3 = "clang bench.ll -o bench -lstdc++ -lm -O1";
			const char* c3 = str3.c_str();
			system(c);
			system(c2);
			system(c3);
	...
	else if (mode == 3)
	{
		for (int i = 0; i <= opt3 * 1 - 1; i++)
		{
			...
			const char* c2 = str2.c_str();
			string str3 = "clang bench.ll -o bench -lstdc++ -lm -O2";
			const char* c3 = str3.c_str();
			system(c);
			system(c2);
			system(c3);
	...
	system("rm bench bench.ll benchmark.ll"); // cleaning after finishing run
	return 0;
}
