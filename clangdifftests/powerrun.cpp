...
int main(int argc, char* argv[])
{
	...
	if (mode == 1)
	{
		...
		{
			string init = "clang -S -o linO0.s linpackm.cpp";
			system(init.c_str());
			string str = "clang";
			...
			string str3 = "clang lin.ll -S";
			const char* c3 = str3.c_str();
			cerr << setw(31) << optimisations[i % opt1] << "  ";
			system(c);
			system(c2);
			system(c3);
			system("diff -y --suppress-common-lines lin.s linO0.s | wc -l");
			system("rm lin.ll lin.s linpackm.s");
		}
	}
	else if (mode == 2)
	{
	...
	// system("rm lin.ll lin.s linpackm.s"); // cleaning after finishing run
	return 0;
}